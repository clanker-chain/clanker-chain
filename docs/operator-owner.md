...
|----------------|------|-----------------|---------------|
| `clanker login` (Privy device grant) | Yes | Yes | Recommended CLI path — same wallet as `/join` |
| Local `~/.clanker/op.key` | Yes | Yes | Self-custody / offline |
| [Privy](https://docs.privy.io/) embedded EOA on `/join` | Yes (browser) | Yes via `clanker login` | Email / passkey in the browser |
...
We do **not** endorse one vendor as identity. Pick the signer that fits your product; pin the same `(chainId, registryAddress)`. Privy is the first vault wired into the CLI; the signer interface can grow.
...
- **CLI** — `clanker login` (recommended) or `clanker setup` with a local key, then `fund` / mint / pair.
...
Approve on [`/authorize`](https://clanker-chain.com/authorize) with the same email. Lost the bot `.key` file? Mint another computer label under the same operator on `/join`.
