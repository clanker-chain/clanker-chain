...
The marketing site (`clanker-chain.com`) is the same Caddy, serving `website/dist`. After pulling site or mqtt-auth changes: `bash scripts/deploy-website.sh` (from repo root: `bash hub/mqtt-service/scripts/deploy-website.sh`). That rebuilds the site **and** mqtt-auth (browser `/pair*` CORS for `/join`). Set `PUBLIC_PRIVY_APP_ID` in `.env` for the join page. Point the apex at this host before the first HTTPS cert can issue.
