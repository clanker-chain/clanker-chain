...
**Facts · Policy · Transport** — This plugin is a **day-one adapter**, not the identity product. EIP-712 verify is **Facts** + signatures (`operator_id` must match on-chain). `dmPolicy` / `allowFrom` / `allowOperators` is this product’s **Policy**. Hub `/acl` is **Transport**. → [`docs/trust-model.md`](../../docs/trust-model.md)
...
- **2026.5.26** — Adds `isNoReply` / `NO_REPLY` suppression on inbound channel `deliver` (first non-empty line marker; SDK flag; JSON wrapper). Does not affect `mqtt_send` / `sendText`. Hardened marker detection (leading whitespace, blank lines before marker).
...
```bash
cd openclaw/mqtt-channel-plugin
npm install
npm run build
```
