...
**Facts · Policy · Transport** — This plugin is a **day-one adapter**, not the identity product. `mqtt_send` signs a **Facts**-bound envelope. It does not grant the right to be delivered; that is Policy + Transport. → [`docs/trust-model.md`](../../docs/trust-model.md)
...
Compare low-level stack behavior with [`hub/mqtt-service/test-connect.mjs`](../../hub/mqtt-service/test-connect.mjs).

## Development

```bash
cd openclaw/mqtt-tools-plugin
