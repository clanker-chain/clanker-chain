...
**On-chain identity (Foundry):** `chain/` — Anvil, `forge test`, `node clanker-cli/bin/clanker.mjs chain up|deploy|mint-*`. For `forge-std`, clone with `--recurse-submodules` or run `git submodule update --init chain/lib/forge-std`.
...
Install both, enable plugin ids **`mqtt`** and **`mqtt-tools`**, set `channels.mqtt` (`botId`, `operatorId`, broker, chain RPC, registry). Bot key: `~/.openclaw/keys/{bot_id}.key` (`0x` + 64 hex secp256k1), from `clanker chain mint-bot`.
