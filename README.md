...
**Operator UX:** [`docs/operator-cli.md`](docs/operator-cli.md) — `npm i -g @clanker-chain/clanker-cli@2026.9.7-4`, `clanker setup`, local `~/.clanker` profile, Anvil guard on public RPCs, inferred operator for bot mint. Bot keys dual-write to `~/.openclaw/keys/` and `~/.clanker/keys/`.
