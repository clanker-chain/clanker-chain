...
### 2. Add identity and keys (on-chain)

- Deploy `ClankerIdentity` and register operators/bots on-chain (see [`chain/README.md`](chain/README.md)).
- On each bot machine:
  - Use the secp256k1 private key at `~/.openclaw/keys/{bot_id}.key` (`0x` + 64 hex), generated by `clanker chain mint-bot`.
  - Verify registration with `identity-node-client` `init()`.
